...
<header class="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-gray-200">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="text-xl font-semibold text-gray-700">Katalog Menu</div>
        </div>
    </div>
</header>
...
<main class="container mx-auto p-4 sm:p-6 lg:p-8">
    <div class="relative rounded-lg overflow-hidden mb-8 h-64">
        ...
        <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <h1 class="text-4xl font-extrabold text-white tracking-wider text-center">Jelajahi Rasa Terbaik</h1>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden group transition-shadow hover:shadow-xl border border-gray-100">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-sm text-gray-500 mt-1 line-clamp-2 h-10">{{ $menu->description }}</p>
            ...
            @else
                <div class="space-y-4 max-h-[50vh] overflow-y-auto pr-2">
                    ...
                    <div class="flex items-center space-x-3 bg-white p-3 rounded-md shadow-sm">
                        <img src="{{ $details['image'] }}" class="w-12 h-12 rounded-md object-cover flex-shrink-0">
                        <div class="flex-grow">
                            <p class="font-semibold text-gray-700 text-sm line-clamp-1">{{ $details['name'] }}</p>
                            ...
                            <div class="flex items-center mt-1 space-x-2">
                                <form action="{{ route('cart.update') }}" method="POST" class="inline">
                                    ...
                                    @if($details['quantity'] > 1)
                                        ...
                                        <button type="submit" class="text-gray-500 hover:text-red-500 bg-gray-200 rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">-</button>
                                    @else
                                        @endif
                                </form>
                                
                                <span class="text-sm font-semibold">{{ $details['quantity'] }}</span>
                                
                                <form action="{{ route('cart.update') }}" method="POST" class="inline">
                                    @csrf @method('PATCH')
                                    <input type="hidden" name="id" value="{{ $id }}">
                                    <input type="hidden" name="quantity" value="{{ $details['quantity'] + 1 }}">
                                    <button type="submit" class="text-gray-500 hover:text-green-500 bg-gray-200 rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">+</button>
                                </form>
                            </div>
                        </div>
                ...
                <div class="mt-8 border-t border-gray-300 pt-6 space-y-2 text-gray-600 text-sm">
                    ...
                    <div class="flex justify-between font-bold text-lg text-gray-800 pt-2 border-t border-gray-200 mt-2">
                        <span>Total</span>
                        <span>Rp {{ number_format($total, 0, ',', '.') }}</span>
                    </div>
                    
                    <button class="w-full bg-dark text-white py-3 rounded-lg mt-4 font-semibold hover:bg-gray-800 transition-colors shadow-lg">
                        Lanjut Pembayaran
                    </button>
                </div>
            @endif
